// src/controllers/n8nWebhookController.js
...
/**
 * Post product data to n8n webhook
 */
export const postToN8n = async (req, res) => {
  try {
    ...
    const validImageIds = (product.images || []).filter(id => mongoose.Types.ObjectId.isValid(id));
    let images = [];
    if (validImageIds.length > 0) {
      images = await Image.find({ _id: { $in: validImageIds }, sellerId: seller._id }).lean();
    }
    ...
    const n8nPayload = {
      ...
      sellerName: seller.fullName || `${seller.firstName || ''} ${seller.lastName || ''}`.trim(),
    ...
    console.log(`✅ n8n webhook called for product ${productId}:`, {
    ...
    res.json({
      ...
      data: { productId: product._id.toString(), productName: product.name, n8nResponse: n8nResponse.data }
    });
...
export const testN8nConnection = async (req, res) => {
  try {
    ...
    console.log('✅ n8n test connection successful:', { status: n8nResponse.status });
